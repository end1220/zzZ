...
namespace Float
{
	public class ModifyOldItemPage : FloatEditorPage
	{
		...
		private void UpdateItem()
		{
			...
			SteamUGC.SetItemTitle(m_UGCUpdateHandle, projectData.modeldata.title);
			SteamUGC.SetItemDescription(m_UGCUpdateHandle, projectData.modeldata.description);
		...
		bool CheckInputInfo()
		{
			if (string.IsNullOrEmpty(projectData.modeldata.title))
			{
				EditorUtility.DisplayDialog("Error", "Title cannot be empty.", "OK");
				return false;
			}
			if (string.IsNullOrEmpty(projectData.modeldata.description))
			{
		...
		private void ReplaceContentWithWorkshopID(ulong workshopID, string oldFolderName)
		{
			...
			ModelAssetBuilder.SaveModelDataToFile(
					...
					projectData.modeldata.description,
